...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
  switch (keycode) {
    ...
    case mac_7:
      if (record->event.pressed) {
        SEND_STRING(SS_LCTRL("c"));  //ctrl c  for copy
      }
      break;
    case mac_8:
      if (record->event.pressed) {
        SEND_STRING( "std::" );  // std:: for C++
      }
      break;
    case mac_9:
      if (record->event.pressed) {
        SEND_STRING(SS_LCTRL("v"));  //ctrl v  for paste / print
      }
      break;
    case mac_10:
      if (record->event.pressed) {
        SEND_STRING( "make primekb/prime_e:xx:dfu" );  //ctrl v  for paste / print
      }
      break;
...
/*  Layer 2
    ...
    [_CC] = LAYOUT(
		...
		KC_CAPS,  KC_INS,    KC_NO,         KC_NO,     mac_6,        mac_5,            KC_GRV,    KC_NUBS,  mac_8,   KC_NO,   KC_NO,    KC_NO,
